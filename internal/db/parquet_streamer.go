...
// StreamToParquetFast writes entries using ultra-fast JSONL method (for ultra_fast_mode)
func (ps *ParquetStreamer) StreamToParquetFast(entries []*LogEntry, processName string) (string, int64, error) {
	if len(entries) == 0 {
		return "", 0, nil
	}
	...
	processDir := filepath.Join(ps.baseDir, processName)
	if err := os.MkdirAll(processDir, 0755); err != nil {
		return "", 0, fmt.Errorf("failed to create process dir: %v", err)
	}

	// Generate filename with timestamp
	timestamp := time.Now().Format("20060102_150405_000000")
	nanoSuffix := fmt.Sprintf("%d", time.Now().UnixNano()%100000)
	fileName := fmt.Sprintf("logs_%s_%s.parquet.jsonl", timestamp, nanoSuffix)
	...
	for _, entry := range entries {
		line := fmt.Sprintf("{\"ts\":\"%s\",\"lvl\":\"%s\",\"src\":\"%s\",\"msg\":\"%s\",\"pid\":\"%s\",\"tid\":\"%s\"}\n",
